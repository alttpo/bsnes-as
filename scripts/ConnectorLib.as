...
class ConnectorWindow {
  ...
  ConnectorWindow() {
    ...
    window.append(vl);
    {
...

      {
        ...
        @lblStatus = GUI::Label();
        lblStatus.text = "";
        hz.append(lblStatus, GUI::Size(100, -1));
      }
  ...
  private void goClicked() {
    ConnectorLib::host = txtHost.text;
    ConnectorLib::port = txtPort.text;
  ...
  void messageUpdated(const string &in msg) {
    lblStatus.text = msg;
  }
}
...
void init() {
  ...
  @ConnectorLib::connector = ConnectorLib::Connector(
    @ConnectorLib::StateUpdated(connectorWindow.stateUpdated),
    @ConnectorLib::MessageUpdated(connectorWindow.messageUpdated),
    ::message
  );
}
